...
def test_theme_manager_defaults_to_modern_wallpaper_theme() -> None:
    manager = ThemeManager()

    assert manager.theme() == LIGHT_THEME
    assert manager.theme().watercolor_spots
...
def test_native_window_corners_require_windows_11(
    ...
    monkeypatch.setattr(_window_chrome.sys, "platform", "win32")
    monkeypatch.setattr(_window_chrome.QApplication, "platformName", lambda: "windows")
    monkeypatch.setattr(
        _window_chrome.sys,
        "getwindowsversion",
        WindowsVersion,
    ...
    policy = _window_chrome.current_window_surface_policy()

    assert policy.opaque_surface
    assert policy.native_corners is expected


def test_windows_10_surface_uses_opaque_square_live_resize(monkeypatch) -> None:
    from pyside6_modern_widgets import modern_window
    ...
    monkeypatch.setattr(modern_window, "current_window_surface_policy", lambda: policy)
    ...
    assert window._surface_policy is policy
    ...
    assert content.width() < initial_content_width
...
def test_windows_11_surface_uses_native_round_corners(monkeypatch) -> None:
    ...
    policy = WindowSurfacePolicy(opaque_surface=True, native_corners=True)
    monkeypatch.setattr(modern_window, "current_window_surface_policy", lambda: policy)
