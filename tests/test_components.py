...
def test_title_bar_menu_button_and_native_context_menu(monkeypatch) -> None:
    window = ModernWindow(theme=STANDARD_LIGHT_THEME)
    ...
    assert title_bar.standardWatercolorAction.isChecked()
    assert not title_bar.modernWatercolorAction.isChecked()
...
@pytest.mark.parametrize(
    ("theme", "standard_theme", "original_theme"),
    (
        (LIGHT_THEME, STANDARD_LIGHT_THEME, ORIGINAL_LIGHT_THEME),
        (DARK_THEME, STANDARD_DARK_THEME, ORIGINAL_DARK_THEME),
    ),
)
def test_title_bar_menu_switches_watercolor_style_without_changing_mode(
    ...
    window.titleBar.standardWatercolorAction.trigger()

    assert window.theme() == standard_theme
    assert window.watercolorStyle() is WatercolorStyle.STANDARD
    assert window.theme().surface == theme.surface
    assert window.titleBar.standardWatercolorAction.isChecked()
...
def test_theme_manager_defaults_to_colorless_standard_theme() -> None:
    manager = ThemeManager()

    assert manager.theme() is STANDARD_LIGHT_THEME
    for theme in (STANDARD_LIGHT_THEME, STANDARD_DARK_THEME):
        assert theme.watercolor_style is WatercolorStyle.STANDARD
        assert theme.watercolor_spots == ()
        assert QColor(theme.watercolor_base).saturation() == 0
    assert QColor(STANDARD_LIGHT_THEME.watercolor_base) == QColor("#F7F7F7")
    assert QColor(STANDARD_DARK_THEME.watercolor_base) == QColor(
        STANDARD_DARK_THEME.surface
    )
...
def test_title_bar_watercolor_menu_updates_global_theme_followers() -> None:
    ...
    try:
        ...
        manager.setTheme(STANDARD_LIGHT_THEME)
...
def test_collapsed_navigation_shows_scrollbar_when_items_overflow() -> None:
    ...
    assert button.mapTo(sidebar, button.rect().topLeft()).x() == 6
    next_button = sidebar.button(1)
    assert next_button is not None
    assert (
        next_button.mapTo(sidebar, next_button.rect().topLeft()).y()
        - button.mapTo(sidebar, button.rect().topLeft()).y()
        == 40
    )
...
def test_expanded_navigation_scrollbar_does_not_cover_active_corners() -> None:
    ...
    active_y_positions = [
        ...
        if image.pixelColor(button.width() // 2, y) == active_background
    ]
    assert (min(active_y_positions), max(active_y_positions)) == (0, 31)
    next_button = sidebar.button(1)
    assert next_button is not None
...
def test_navigation_visible_outer_spacing_is_balanced() -> None:
    toggle_background = QColor("#FF00FF")
    active_background = QColor("#00FF00")
    theme = replace(
        LIGHT_THEME,
        navigation_background="#FFFFFF",
        control_hover=toggle_background.name(),
        control_pressed=active_background.name(),
    )
    sidebar = NavigationSidebar(theme=theme)
    sidebar.addItem("Top")
    bottom_index = sidebar.addItem("Bottom", position=NavigationPosition.BOTTOM)
    sidebar.setCurrentIndex(bottom_index)
    sidebar.setCollapsed(True, animated=False)
    ...
    toggle_x = sidebar.toggleButton.width() // 2
    toggle_background_y = [
        y
        for y in range(toggle_image.height())
        if toggle_image.pixelColor(toggle_x, y) == toggle_background
    ]
    bottom_button = sidebar.button(bottom_index)
    assert bottom_button is not None
    bottom_image = bottom_button.grab().toImage()
    bottom_x = bottom_button.width() // 2
    bottom_background_y = [
        y
        for y in range(bottom_image.height())
...
def test_navigation_focus_is_borderless_and_keeps_toggle_icon_centered() -> None:
    ...
    assert (min(background_x_positions), max(background_x_positions)) == (0, 35)
    assert (min(background_y_positions), max(background_y_positions)) == (0, 31)
...
def test_theme_manager_can_follow_application_palette() -> None:
    ...
    try:
        ...
        _application().processEvents()
        assert manager.theme() is STANDARD_DARK_THEME
    finally:
        manager.setTheme(STANDARD_LIGHT_THEME)
